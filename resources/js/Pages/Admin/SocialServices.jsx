...
export default function SocialServices({
    ...
    const [search, setSearch] = useRemember("", "admin-social-services-search");
    ...
    const openEditModal = (socialService) => {
    ...
    const openDeleteModal = (socialService) => {
        setSocialServicePendingDelete(socialService);
        setIsDeleteModalOpen(true);
    };

    const closeDeleteModal = () => {
        setSocialServicePendingDelete(null);
        setIsDeleteModalOpen(false);
    };

    const openPdfPreview = (socialService) => {
        if (!socialService?.pdf) {
            return;
        }

        setPdfPreview({
            title: socialService.title,
            url: `/storage/documents/pdfs/${socialService.pdf}`,
        });
    };
    ...
    const submitSocialService = (event) => {
        ...
        const isEditing = Boolean(editingSocialService);
        ...
        openConfirmation({
            ...
            confirmLabel: isEditing ? "Save Changes" : "Add Social Service",
            onConfirm: () => {
                    ...
                    router.post(
                        ...
                        data,
                        {
                            ...
                            onSuccess: () => {
                                ...
                                showNotification(
                                    "success",
                                    "Social service content was updated successfully.",
                                );
                            },
                            onError: () => {
                                showNotification(
                                    "error",
                                    "The social service item could not be updated. Please review the form and try again.",
                                );
                            },
                        },
                    );
                ...
                post(route("admin.social-services.store"), {
                    forceFormData: true,
                    preserveScroll: true,
                    onSuccess: () => {
                        closeFormModal();
                        showNotification(
                            "success",
                            "New social service content was added successfully.",
                        );
                    },
                    onError: () => {
                        showNotification(
                            "error",
                            "The new social service item could not be added. Please review the form and try again.",
                        );
                    },
                });
            },
    ...
    const deleteSocialService = () => {
        if (!socialServicePendingDelete) {
            return;
        }
                ...
                <div className="flex max-h-[calc(100dvh-2rem)] flex-col overflow-hidden bg-white sm:max-h-[calc(100dvh-3rem)]">
                    ...
                    >
                        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-5 sm:px-6">
                            ...
                            <div>
                                <InputLabel
                                    htmlFor="social-services-title"
                                    value="Social Service Title"
                                />
                                <TextInput
                            ...
                            <div>
                                <InputLabel
                                    htmlFor="social-services-image"
                                    value="Image File"
                                />
                                <input
                                    id="social-services-image"
                                    type="file"
                                    ...
                                    disabled={processing}
                                />
                                {editingSocialService?.image ? (
                                    <p className="mt-2 text-xs text-gray-500">
                                        Current image: {editingSocialService.image}
                                    </p>
                                ) : null}
                                <InputError
                            ...
                            <div>
                                <InputLabel
                                    htmlFor="social-services-pdf"
                                    value="PDF File"
                                />
                                <input
                                    id="social-services-pdf"
                                    type="file"
                                    ...
                                    disabled={processing}
                                />
                                {editingSocialService?.pdf ? (
                                    <p className="mt-2 text-xs text-gray-500">
                                        Current PDF: {editingSocialService.pdf}
                                    </p>
                                ) : (
                                    <p className="mt-2 text-xs text-gray-500">
                            ...
                            <div>
                                <InputLabel
                                    htmlFor="social-services-description"
                                    value="Description"
                                />
                                <textarea
                                    id="social-services-description"
                        ...
                        <div className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-t border-slate-200 bg-slate-50 px-5 py-4 shadow-[0_-8px_24px_rgba(15,23,42,0.06)] sm:px-6">
                            <div>
                                {editingSocialService ? (
                                    <DangerButton
                                        type="button"
                                        onClick={() =>
                                            openDeleteModal(editingSocialService)
                                        }
                                        disabled={processing}
                                    >
                                        Delete
            ...
            >
                <div className="p-6">
                    <h2 className="text-xl font-semibold text-slate-950">
                        Delete Social Service Item
                    </h2>
                    <p className="mt-3 text-sm leading-6 text-slate-600">
                        {socialServicePendingDelete
                            ? `Are you sure you want to delete "${socialServicePendingDelete.title}"? This action cannot be undone.`
                            : "Are you sure you want to delete this social service item?"}
                    </p>
