...
export default function Tourism({
    ...
    contents: tourismItems = [],
}) {
    const [contents, setContents] = useState(tourismItems);
    ...
    const [search, setSearch] = useRemember("", "admin-tourism-search");
    ...
    const findTabById = (tabId) =>
        tourismTabs.find((tab) => tabMatches(tab, tabId)) ?? null;

    const normalizeTabId = (tabId) => findTabById(tabId)?.id ?? tabId;

    useEffect(() => {
        setContents(tourismItems);
    }, [tourismItems]);
    ...
    const openEditModal = (tourismContent) => {
        setEditingTourism(tourismContent);
        clearErrors();
        setData({
            tab_id: normalizeTabId(tourismContent.tab_id ?? activeTab),
    ...
    const openDeleteModal = (tourismContent) => {
        setTourismPendingDelete(tourismContent);
        setIsDeleteModalOpen(true);
    };

    const closeDeleteModal = () => {
        setTourismPendingDelete(null);
        setIsDeleteModalOpen(false);
    };

    const openPdfPreview = (tourismContent) => {
        if (!tourismContent?.pdf) {
            return;
        }

        setPdfPreview({
            title: tourismContent.title,
            url: `/storage/documents/pdfs/${tourismContent.pdf}`,
        });
    };
    ...
    const submitTourism = (event) => {
        ...
        const isEditing = Boolean(editingTourism);

        openConfirmation({
            title: isEditing
                ? "Save changes to tourism item?"
                : "Add new tourism item?",
            message: isEditing
                ? "Are you sure you want to save the updates to this tourism item?"
                : "Are you sure you want to add this new tourism item to the public Tourism page?",
            confirmLabel: isEditing ? "Save Changes" : "Add Tourism",
            onConfirm: () => {
                if (editingTourism) {
                    router.post(
                        route("admin.tourism.update", editingTourism.id),
                        data,
                        {
                            forceFormData: true,
                            preserveScroll: true,
                            onSuccess: () => {
                                ...
                                showNotification(
                                    "success",
                                    "Tourism content was updated successfully.",
                                );
                            },
                            onError: () => {
                                showNotification(
                                    "error",
                                    "The tourism item could not be updated. Please review the form and try again.",
                                );
                            },
                        },
                    );
                ...
                post(route("admin.tourism.store"), {
                    forceFormData: true,
                    preserveScroll: true,
                    onSuccess: () => {
                        closeFormModal();
                        showNotification(
                            "success",
                            "New tourism content was added successfully.",
                        );
                    },
                    onError: () => {
                        showNotification(
                            "error",
                            "The new tourism item could not be added. Please review the form and try again.",
                        );
                    },
                });
            },
    ...
    const deleteTourism = () => {
        if (!tourismPendingDelete) {
            return;
        }

        destroy(route("admin.tourism.destroy", tourismPendingDelete.id), {
            preserveScroll: true,
            onSuccess: () => {
                closeDeleteModal();
                showNotification(
                    "success",
                    "Tourism content was deleted successfully.",
                );

                if (editingTourism?.id === tourismPendingDelete.id) {
                    closeFormModal();
                }
            },
            onError: () => {
                showNotification(
                    "error",
                    "The tourism item could not be deleted. Please try again.",
                );
            },
        });
    };
                            ...
                            <div className="min-w-0 space-y-3">
                                <div className="space-y-1">
                                    ...
                                    <p className="max-w-2xl text-sm leading-6 text-slate-600">
                                        Manage the fixed tourism tabs for events,
                                        festivities, and tourism sites shown on
                                        the public page.
                                    </p>
                                </div>
                            ...
                            <div className="flex w-full min-w-0 flex-col gap-3 sm:flex-row xl:w-auto xl:min-w-[28rem]">
                                <div className="relative min-w-0 w-full sm:flex-1 xl:w-80 xl:flex-none">
                                    ...
                                    <input
                                        ...
                                        placeholder={`Search ${currentTab?.label?.toLowerCase() ?? "tourism items"}`}
                                    />
                                </div>
                                <button
                                    type="button"
                        ...
                        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-5 sm:px-6">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    disabled={processing}
                                />
                                {editingTourism?.image ? (
                                    <p className="mt-2 text-xs text-gray-500">
                                        Current image: {editingTourism.image}
                                    </p>
                                ) : null}
                                <InputError
                            ...
                            <div>
                                ...
                                {editingTourism?.pdf ? (
                                    <p className="mt-2 text-xs text-gray-500">
                                        Current PDF: {editingTourism.pdf}
                                    </p>
                                ) : (
                                    <p className="mt-2 text-xs text-gray-500">
                            ...
                            <div>
                                <InputLabel
                                    htmlFor="tourism-description"
                                    value="Description"
                                />
                                <textarea
                                    id="tourism-description"
                            ...
                            <div>
                                {editingTourism ? (
                                    <DangerButton
                                        type="button"
                                        onClick={() =>
                                            openDeleteModal(editingTourism)
                                        }
                                        disabled={processing}
                                    >
                                        Delete
            ...
            >
                <div className="p-6">
                    ...
                    <p className="mt-3 text-sm leading-6 text-slate-600">
                        {tourismPendingDelete
                            ? `Are you sure you want to delete "${tourismPendingDelete.title}"? This action cannot be undone.`
                            : "Are you sure you want to delete this tourism item?"}
                    </p>
