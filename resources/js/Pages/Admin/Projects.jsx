...
export default function Projects({
    ...
    contents: projectItems = [],
}) {
    const [contents, setContents] = useState(projectItems);
    ...
    const [search, setSearch] = useRemember("", "admin-projects-search");
    ...
    const findTabById = (tabId) =>
        projectTabs.find((tab) => tabMatches(tab, tabId)) ?? null;

    const normalizeTabId = (tabId) => findTabById(tabId)?.id ?? tabId;

    useEffect(() => {
        setContents(projectItems);
    }, [projectItems]);
    ...
    const openEditModal = (project) => {
        ...
        clearErrors();
        setData({
            tab_id: normalizeTabId(project.tab_id ?? activeTab),
            title: project.title ?? "",
            image: null,
            pdf: null,
    ...
    const openPdfPreview = (project) => {
        ...
        setPdfPreview({
            title: project.title,
            url: `/storage/documents/pdfs/${project.pdf}`,
        });
    };
    ...
    const submitProject = (event) => {
        ...
        openConfirmation({
            title: isEditing
                ...
                : "Add new project?",
            message: isEditing
                ? "Are you sure you want to save the updates to this project item?"
                : "Are you sure you want to add this new project item to the public Projects page?",
            confirmLabel: isEditing ? "Save Changes" : "Add Project",
            onConfirm: () => {
                if (editingProject) {
                    router.post(
                        ...
                        data,
                        {
                            ...
                            onSuccess: () => {
                                ...
                                showNotification(
                                    "success",
                                    "Project content was updated successfully.",
                                );
                            },
                            onError: () => {
                                showNotification(
                                    "error",
                                    "The project item could not be updated. Please review the form and try again.",
                                );
                            },
                        },
                    );
                ...
                post(route("admin.projects.store"), {
                    ...
                    onSuccess: () => {
                        ...
                        showNotification(
                            "success",
                            "New project content was added successfully.",
                        );
                    },
                    onError: () => {
                        showNotification(
                            "error",
                            "The new project item could not be added. Please review the form and try again.",
                        );
                    },
                });
            },
    ...
    const deleteProject = () => {
        ...
        destroy(route("admin.projects.destroy", projectPendingDelete.id), {
            preserveScroll: true,
            onSuccess: () => {
                closeDeleteModal();
                showNotification(
                    "success",
                    "Project content was deleted successfully.",
                );

                if (editingProject?.id === projectPendingDelete.id) {
                    closeFormModal();
                }
            },
            onError: () => {
                showNotification(
                    "error",
                    "The project item could not be deleted. Please try again.",
                );
            },
        });
    };
                            ...
                            <div className="min-w-0 space-y-3">
                                <div className="space-y-1">
                                    ...
                                    <p className="max-w-2xl text-sm leading-6 text-slate-600">
                                        Manage the fixed project tabs used by the
                                        public interface. Each record is stored
                                        directly under either Completed or
                                        On-Going.
                                    </p>
                                </div>
                            ...
                            <div className="flex w-full min-w-0 flex-col gap-3 sm:flex-row xl:w-auto xl:min-w-[28rem]">
                                <div className="relative min-w-0 w-full sm:flex-1 xl:w-80 xl:flex-none">
                                    ...
                                    <input
                                        ...
                                        placeholder={`Search ${currentTab?.label?.toLowerCase() ?? "project items"}`}
                                    />
                                </div>
                                <button
                                    type="button"
                    ...
                    >
                        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-5 sm:px-6">
                            ...
                            <div>
                                ...
                                {editingProject?.image ? (
                                    <p className="mt-2 text-xs text-gray-500">
                                        Current image: {editingProject.image}
                                    </p>
                                ) : null}
                                <InputError
                            ...
                            <div>
                                ...
                                {editingProject?.pdf ? (
                                    <p className="mt-2 text-xs text-gray-500">
                                        Current PDF: {editingProject.pdf}
                                    </p>
                                ) : (
                                    <p className="mt-2 text-xs text-gray-500">
                            ...
                            <div>
                                {editingProject ? (
                                    <DangerButton
                                        type="button"
                                        onClick={() =>
                                            openDeleteModal(editingProject)
                                        }
                                        disabled={processing}
                                    >
                                        Delete
                ...
                <div className="p-6">
                    ...
                    <p className="mt-3 text-sm leading-6 text-slate-600">
                        {projectPendingDelete
                            ? `Are you sure you want to delete "${projectPendingDelete.title}"? This action cannot be undone.`
