...
const Tourism = () => {
    const tabs = [
        ...
        { id: "festivities", label: "Festivities", icon: "festivities" },
    ...
    const renderIcon = (icon, isActive) => {
        ...
        if (icon === "events") {
            return <IoMegaphoneOutline className={`h-4 w-4 ${iconClassName}`} />;
        }
        if (icon === "festivities") {
            return <IoColorPaletteOutline className={`h-4 w-4 ${iconClassName}`} />;
        }
        return <IoLocationOutline className={`h-4 w-4 ${iconClassName}`} />;
    };
