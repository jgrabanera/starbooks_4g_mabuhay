...
import { IoCalendarOutline, IoFolderOpenOutline, IoSearchOutline } from "react-icons/io5";

const resourceItems = [
    { id: 1, title: "Citizen charter handbook for basic frontline municipal services.", reference: "RES-001", date: "January 12, 2026" },
    { id: 2, title: "Sample download package for local forms, request slips, and office references.", reference: "RES-002", date: "February 3, 2026" },
    { id: 3, title: "Public information guide for office transactions and general requirements.", reference: "RES-003", date: "February 27, 2026" },
    { id: 4, title: "Dummy reference list for agriculture, health, and community development materials.", reference: "RES-004", date: "March 14, 2026" },
    { id: 5, title: "Local service directory for key municipal offices and assistance channels.", reference: "RES-005", date: "April 8, 2026" },
    { id: 6, title: "Community preparedness resource pack for barangay-level awareness sessions.", reference: "RES-006", date: "May 19, 2026" },
];
...
const LGUResources = () => {
    ...
    const filteredItems = resourceItems.filter((item) => {
        const searchableContent =
            `${item.title} ${item.reference} ${item.date}`.toLowerCase();
            ...
            <div className="mx-auto flex min-h-[78vh] max-w-5xl flex-col">
                                ...
                                <div className="relative z-10 grid min-h-[10.25rem] gap-3 p-4 md:grid-cols-[minmax(0,1fr)_6rem]">
                                    ...
                                    <div className="mt-auto hidden overflow-hidden rounded-xl border border-white/80 shadow-sm md:block">
                                        <div className="flex h-24 w-full items-center justify-center bg-[linear-gradient(135deg,rgba(5,150,105,0.92),rgba(16,185,129,0.78),rgba(250,204,21,0.62))]">
                                            <IoFolderOpenOutline className="h-10 w-10 text-white" />
                                        </div>
                                    </div>
                                </div>
                ...
                {filteredItems.length > 0 && totalPages > 1 ? (
                    <div className="mt-auto flex flex-wrap items-center justify-center gap-2 pt-8">
                        <button type="button" onClick={() => setCurrentPage((page) => Math.max(1, page - 1))} disabled={currentPage === 1} className="rounded-full border border-emerald-200 bg-white/85 px-4 py-2 text-sm font-semibold text-emerald-800 shadow-md transition hover:bg-emerald-50 disabled:cursor-not-allowed disabled:opacity-50">Previous</button>
                        {Array.from({ length: totalPages }, (_, index) => {
                            const pageNumber = index + 1;
                            const isActive = currentPage === pageNumber;
                            return (
                                <button key={pageNumber} type="button" onClick={() => setCurrentPage(pageNumber)} className={`flex h-10 w-10 items-center justify-center rounded-full border text-sm font-bold shadow-md transition ${isActive ? "border-emerald-700 bg-emerald-700 text-white" : "border-emerald-200 bg-white/85 text-emerald-800 hover:bg-emerald-50"}`}>
                                    {pageNumber}
                                </button>
                            );
                        })}
                        <button type="button" onClick={() => setCurrentPage((page) => Math.min(totalPages, page + 1))} disabled={currentPage === totalPages} className="rounded-full border border-emerald-200 bg-white/85 px-4 py-2 text-sm font-semibold text-emerald-800 shadow-md transition hover:bg-emerald-50 disabled:cursor-not-allowed disabled:opacity-50">Next</button>
                    </div>
                ) : null}
                ...
                {filteredItems.length === 0 ? (
                    <div className="mt-8 rounded-[1.5rem] border border-dashed border-emerald-300 bg-white/65 px-5 py-8 text-center shadow-md">
                        <p className="text-base font-bold text-emerald-950">No resources found.</p>
                        <p className="mt-2 text-sm text-slate-600">Try a different keyword or clear the search field.</p>
                    </div>
                ) : null}
            </div>
        </div>
