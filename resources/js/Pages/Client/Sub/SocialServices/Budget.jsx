...
import { IoCalendarOutline, IoCashOutline, IoSearchOutline } from "react-icons/io5";

const budgetItems = [
    { id: 1, title: "Illustrative annual budget summary for municipal operations and local programs.", reference: "BDG-001", date: "January 20, 2026" },
    { id: 2, title: "Sample budget allocation note for infrastructure, health, and social support initiatives.", reference: "BDG-002", date: "February 18, 2026" },
    { id: 3, title: "Dummy budget posting for office maintenance, supplies, and service delivery support.", reference: "BDG-003", date: "March 25, 2026" },
    { id: 4, title: "Reference budget entry for special development funds and barangay-linked assistance.", reference: "BDG-004", date: "April 29, 2026" },
];
...
const Budget = () => {
    ...
    const filteredItems = budgetItems.filter((item) => `${item.title} ${item.reference} ${item.date}`.toLowerCase().includes(searchTerm.trim().toLowerCase()));
        ...
        <div className="rounded-[1.75rem] border border-white/80 bg-white/55 p-4 shadow-lg backdrop-blur-md md:p-6">
            <div className="mx-auto flex min-h-[78vh] max-w-5xl flex-col">
                <div className="text-center">
                    ...
                    <h1 className="mt-3 text-3xl font-black uppercase tracking-[0.06em] text-emerald-950 md:text-4xl">Budget</h1>
                </div>
                <div className="mx-auto mt-6 w-full md:max-w-xl">
                    <label htmlFor="budget-search" className="sr-only">Search budget items</label>
                    <div className="flex w-full items-center gap-2 rounded-full border border-emerald-200 bg-white/90 px-4 py-2 text-emerald-800 shadow-md">
                        ...
                        <input id="budget-search" type="text" value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="Search budget items" className="w-full max-w-6xl border-none bg-transparent text-sm font-medium placeholder:text-emerald-700/70 focus:outline-none focus:ring-0" />
                    </div>
                </div>
                            ...
                            <div className="relative z-10 grid min-h-[10.25rem] gap-3 p-4 md:grid-cols-[minmax(0,1fr)_6rem]">
                                ...
                                <div className="mt-auto hidden overflow-hidden rounded-xl border border-white/80 shadow-sm md:block">
                                    <div className="flex h-24 w-full items-center justify-center bg-[linear-gradient(135deg,rgba(5,150,105,0.92),rgba(16,185,129,0.78),rgba(250,204,21,0.62))]">
                                        <IoCashOutline className="h-10 w-10 text-white" />
                                    </div>
                                </div>
                            </div>
                ...
                {filteredItems.length === 0 ? <div className="mt-8 rounded-[1.5rem] border border-dashed border-emerald-300 bg-white/65 px-5 py-8 text-center shadow-md"><p className="text-base font-bold text-emerald-950">No budget items found.</p><p className="mt-2 text-sm text-slate-600">Try a different keyword or clear the search field.</p></div> : null}
            </div>
        </div>
    );
};
