...
import { IoCalendarOutline, IoReaderOutline, IoSearchOutline } from "react-icons/io5";

const ordinanceItems = [
    { id: 1, title: "Sample municipal ordinance for sanitation, cleanliness, and public compliance standards.", reference: "ORD-001", date: "February 8, 2026" },
    { id: 2, title: "Illustrative ordinance entry on local safety zones and regulated public activity areas.", reference: "ORD-002", date: "March 19, 2026" },
    { id: 3, title: "Dummy ordinance posting for market operations, vendor compliance, and public order.", reference: "ORD-003", date: "May 7, 2026" },
    { id: 4, title: "Reference ordinance for environmental practices and solid waste management guidance.", reference: "ORD-004", date: "June 24, 2026" },
];
...
const Ordinance = () => {
    ...
    const filteredItems = ordinanceItems.filter((item) => `${item.title} ${item.reference} ${item.date}`.toLowerCase().includes(searchTerm.trim().toLowerCase()));
        ...
        <div className="rounded-[1.75rem] border border-white/80 bg-white/55 p-4 shadow-lg backdrop-blur-md md:p-6">
            <div className="mx-auto flex min-h-[78vh] max-w-5xl flex-col">
                <div className="text-center">
                    ...
                    <h1 className="mt-3 text-3xl font-black uppercase tracking-[0.06em] text-emerald-950 md:text-4xl">Ordinance</h1>
                </div>
                <div className="mx-auto mt-6 w-full md:max-w-xl">
                    <label htmlFor="ordinance-search" className="sr-only">Search ordinances</label>
                    <div className="flex w-full items-center gap-2 rounded-full border border-emerald-200 bg-white/90 px-4 py-2 text-emerald-800 shadow-md">
                        ...
                        <input id="ordinance-search" type="text" value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="Search ordinances" className="w-full max-w-6xl border-none bg-transparent text-sm font-medium placeholder:text-emerald-700/70 focus:outline-none focus:ring-0" />
                    </div>
                </div>
                            ...
                            <div className="relative z-10 grid min-h-[10.25rem] gap-3 p-4 md:grid-cols-[minmax(0,1fr)_6rem]">
                                ...
                                <div className="mt-auto hidden overflow-hidden rounded-xl border border-white/80 shadow-sm md:block">
                                    <div className="flex h-24 w-full items-center justify-center bg-[linear-gradient(135deg,rgba(5,150,105,0.92),rgba(16,185,129,0.78),rgba(250,204,21,0.62))]">
                                        <IoReaderOutline className="h-10 w-10 text-white" />
                                    </div>
                                </div>
                            </div>
                ...
                {filteredItems.length === 0 ? <div className="mt-8 rounded-[1.5rem] border border-dashed border-emerald-300 bg-white/65 px-5 py-8 text-center shadow-md"><p className="text-base font-bold text-emerald-950">No ordinances found.</p><p className="mt-2 text-sm text-slate-600">Try a different keyword or clear the search field.</p></div> : null}
            </div>
        </div>
    );
};
