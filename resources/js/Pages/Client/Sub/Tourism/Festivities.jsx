...
import { IoCalendarOutline, IoColorPaletteOutline, IoSearchOutline } from "react-icons/io5";

const festivityItems = [
    { id: 1, title: "Sample municipal festivity profile for local heritage, music, and cultural street activity.", reference: "FST-001", date: "September 12, 2026" },
    { id: 2, title: "Dummy festivity posting for barangay dance presentations and food fair exhibits.", reference: "FST-002", date: "October 3, 2026" },
    { id: 3, title: "Illustrative culture celebration entry highlighting thanksgiving and community traditions.", reference: "FST-003", date: "November 14, 2026" },
    { id: 4, title: "Reference local festival content for visitor schedules and cultural feature activities.", reference: "FST-004", date: "December 8, 2026" },
];
...
const Festivities = () => {
    ...
    const filteredItems = festivityItems.filter((item) => `${item.title} ${item.reference} ${item.date}`.toLowerCase().includes(searchTerm.trim().toLowerCase()));
        ...
        <div className="rounded-[1.75rem] border border-white/80 bg-white/55 p-4 shadow-lg backdrop-blur-md md:p-6">
            <div className="mx-auto flex min-h-[78vh] max-w-5xl flex-col">
                <div className="text-center">
                    ...
                    <h1 className="mt-3 text-3xl font-black uppercase tracking-[0.06em] text-emerald-950 md:text-4xl">Festivities</h1>
                </div>
                <div className="mx-auto mt-6 w-full md:max-w-xl">
                    <label htmlFor="festivities-search" className="sr-only">Search festivities</label>
                    <div className="flex w-full items-center gap-2 rounded-full border border-emerald-200 bg-white/90 px-4 py-2 text-emerald-800 shadow-md">
                        ...
                        <input id="festivities-search" type="text" value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="Search festivities" className="w-full max-w-6xl border-none bg-transparent text-sm font-medium placeholder:text-emerald-700/70 focus:outline-none focus:ring-0" />
                    </div>
                </div>
                            ...
                            <div className="relative z-10 grid min-h-[10.25rem] gap-3 p-4 md:grid-cols-[minmax(0,1fr)_6rem]">
                                ...
                                <div className="mt-auto hidden overflow-hidden rounded-xl border border-white/80 shadow-sm md:block">
                                    <div className="flex h-24 w-full items-center justify-center bg-[linear-gradient(135deg,rgba(5,150,105,0.92),rgba(16,185,129,0.78),rgba(250,204,21,0.62))]">
                                        <IoColorPaletteOutline className="h-10 w-10 text-white" />
                                    </div>
                                </div>
                            </div>
                ...
                {filteredItems.length === 0 ? <div className="mt-8 rounded-[1.5rem] border border-dashed border-emerald-300 bg-white/65 px-5 py-8 text-center shadow-md"><p className="text-base font-bold text-emerald-950">No festivities found.</p><p className="mt-2 text-sm text-slate-600">Try a different keyword or clear the search field.</p></div> : null}
            </div>
        </div>
    );
};
